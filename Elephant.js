//using elephant sql as database and performing functionality of sending mails to employees whose birthday is today.

import pg from "pg"; //node postgress
import dotenv from "dotenv"; //loading environment variables

// integrating mailer send.
import { MailerSend, EmailParams, Sender, Recipient } from 'mailersend'; //importing

dotenv.config();

const connesctionUrl = process.env.CONN_URL; //elephant sql database url

const mailerSend = new MailerSend({ //creating an instance and storing the value of mailer send api key
    apiKey: process.env.MAILERSEND_API_KEY
})

const sendFrom = new Sender('nishant@gili.pro', 'Nishant Krishna Ghadigaonkar'); //setting sender name
...
client.connect((error) => { //connecting with database
    ...
    client.query("select * from TestEmployees where to_char(dob, 'MM-DD') = to_char(current_date, 'mm-dd')",
        (err, result) => {
            if (err) {
                console.log('Error while executing query');
            }
            let empData = result.rows;
            empData.forEach((employee) => { //targeting each employee separately
